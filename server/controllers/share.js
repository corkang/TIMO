...
exports.getTimetable = async (req, res) => {
  JWT.verify(req.params.id, process.env.JWT_SECRET, async (err, payload) => {
    if (err) return res.sendStatus(404);

    const timetableId = typeof payload === 'object' ? payload.id : payload;
    if (!timetableId) return res.sendStatus(404);
